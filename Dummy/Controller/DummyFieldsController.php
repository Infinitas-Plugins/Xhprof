...
class DummyFieldsController extends DummyAppController {	
	function admin_index($dummy_table_id) {
		...
		if (empty($data['DummyField'])) {
			...
			if ($field_count == 0) {
				$this->Session->setFlash(__('No fields. Analyze database'));
				$this->redirect(array('action' => 'analyze_all', 'admin' => true));
			} else {
				$this->Session->setFlash(__('No fields for this table. Analyze table'));
				$this->redirect(array(
						'action' => 'analyze', 
						$dummy_table_id, 
						'admin' => true));
	...
	function admin_deactivate($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid Field.'));
			$this->redirect(array(
		...
		if ($this->DummyField->save($field, false)) {
			$this->Session->setFlash(__('Field deactivated'));
		} else {
			$this->Session->setFlash(__('Field save failed.'));
		}
		$this->redirect(array(
				'action' => 'index', 
	...
	function admin_activate($id = null) {
		...
		if ($this->DummyField->save($field, false)) {
			$this->Session->setFlash(__('Field activated'));
		} else {
			$this->Session->setFlash(__('Field save failed.'));
		}
		$this->redirect(array(
	...
	function admin_analyze_all() {
		if ($this->DummyField->analyze()) {
			$this->Session->setFlash(__('Fields analyzed.'));
			$this->redirect(array(
					'controller' => 'dummy_tables', 
					'action' => 'index', 
					'admin' => true));
	...
	function admin_change($dummy_table_id) {
		if (!empty($this->data)) {
			if ($this->DummyField->save($this->data, false)) {
				$this->Session->setFlash(__('Field updated.'));
			} else {
				$this->Session->setFlash(__('Field update failed!'));
			}
		}
		$this->redirect(array('action' => 'index', $dummy_table_id, 'admin' => true));
	}
