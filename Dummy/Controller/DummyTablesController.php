...
class DummyTablesController extends AppController {	
	function admin_index() {
		...
		if (empty($tables)) {
			$this->Session->setFlash(__('No tables. Analyze database'));
	...
	function admin_view($tablename = null) {
		if (!$tablename) {
			$this->Session->setFlash(__('Invalid Table.'));
	...
	function admin_generate_all() {
		...
		foreach ($tables as $table) {
			...
			if (!$this->DummyTable->generate()) {
				$this->Session->setFlash(__('Failed to generate table : ') . $table['DummyTable']['name']);
				$this->redirect(array('action' => 'index', 'admin' => true));
			}
		}
		
		$this->Session->setFlash(__('Generated Tables.'));
		$this->redirect(array('action' => 'index', 'admin' => true));
	}
	...
	function admin_number($id = null) {
		...
		if ($this->DummyTable->save($this->data)) {
			$this->Session->setFlash(__('Generation numbers updated for :') . $id);	
		}

		else {
			$this->Session->setFlash(__('Failed to update number : ') . $id);
		}
	...
	function admin_generate($id = null) {
		...
		if (!$this->DummyTable->data['DummyTable']['active']) {
			$this->Session->setFlash(__('Table inactive.'));
		...
		if ($number_rows_generated > 0) {
			$this->Session->setFlash(sprintf(__('Generated %s of %s rows.'), $number_rows_generated, $this->DummyTable->data['DummyTable']['number']));
			$this->redirect(array('controller' => 'dummy_fields', 'action' => 'index', $id));
		}

		else {
			$this->Session->setFlash(__('Failed to generate any rows.'));
		}
	...
	function admin_analyze($id) {
		if ($this->DummyTable->analyze($id)) {
			$this->Session->setFlash(__('Tables analyzed.'));
			$this->redirect(array('controller' => 'dummy_fields', 'action' => 'index', $id));
		}
	}
	...
	function admin_deactivate($id = null) {
		...
		if ($this->DummyTable->save($table, false)) {
			$this->Session->setFlash(__('Table deactivated'));
		}

		else {
			$this->Session->setFlash(__('Table save failed.'));
		}
	...
	function admin_truncate($id = null) {
		...
		if ($this->DummyTable->query('TRUNCATE ' . $tablename['DummyTable']['table'])) {
			$this->Session->setFlash(__('Could not empty table.'));
		}

		else {

			$this->Session->setFlash(__('Table emptied!'));
		}
