				...
				if (!isset($table['active']) || (isset($table['active']) && $table['active']) ) {
					?>
						<tr class="<?php echo $this->Infinitas->rowClass(); ?>">
							...
							<td>
								<?php
									 echo $html->link(__('Generate'), array('action'=>'generate', $table['id']));
								if ($editable) {
									echo ' '.$html->link(__('Deactivate'), array('action'=>'deactivate', $table['id']));
								}
								?>
							</td>
						</tr>
							...
							if (!$table['active']) { ?>
								<tr class="<?php echo $this->Infinitas->rowClass(); ?>">
									...
									<td>
										<?php
											if ($editable) {
												echo $html->link(__('Activate'), array('action'=>'activate', $table['id']));
											}
										?>&nbsp;
									</td>
								</tr><?php
